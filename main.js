...
//Gets the stations with the most lines
app.get('/station/most_lines',(req, res)=>{
    connection.query('SELECT * FROM stogAll WHERE number_of_lines = (SELECT MAX(number_of_lines) FROM stogAll);',
        (error, results) => {
            if (error) {
                res.status(500).send('An error occurred while fetching data.');
            } else {
...
//lines length
app.get('/:line/length',(req, res)=> {
    const stogLine = req.params.line
    connection.query('SELECT COUNT(*) AS length FROM stogAll WHERE `lines` LIKE ?',
        ["%" + stogLine + "%"],
        (error, results) => {
            if (error) {
                res.status(500).send('An error occurred while fetching data.');
